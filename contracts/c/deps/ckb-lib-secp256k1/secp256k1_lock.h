...
int extract_witness_input_type(uint8_t *witness, uint64_t len,
                         mol_seg_t *input_type_bytes_seg) {
  mol_seg_t witness_seg;
  witness_seg.ptr = witness;
  witness_seg.size = len;
...
int get_secp256k1_blake160_sighash_all(
    unsigned char pubkey_hash_out[BLAKE160_SIZE],
    size_t input_index,
    size_t source) {
  int ret;
  ...
  ret = ckb_load_witness(temp, &witness_len, 0, input_index, source);
  if (ret != CKB_SUCCESS) {
...
int verify_secp256k1_blake160_sighash_all(
  ...
  unsigned char temp[BLAKE160_SIZE];
  int ret = get_secp256k1_blake160_sighash_all(temp, 0, CKB_SOURCE_GROUP_INPUT);
  if (ret != 0) {
    return ret;
  }
